...
const std::vector<std::string> FileHandler::labelFolderNames =
{
...
{
	// to move an image, there is no need to open the old one, save to a new location, and delete the old instance
	// instead, moving can be a simple renaming operation
	...
	for (int i = 0; i < pathInfosOld.size(); i++)
	{
		...
		std::string newPath = 
			...
			pathInfosOld[i].labelFolderName + std::string("\\") +
...
int FileHandler::readLabelFolderImages(std::string labelFolderBasePath, std::vector<std::pair<Mat_<Vec3b>, std::string>>& imagesAndLabels, std::map<std::string, int>& labelsAndCounts)
{
	for (std::string encoding : labelFolderNames)
	{
		labelsAndCounts.insert({ encoding, 0 });
	}

	for (std::string encoding : labelFolderNames)
	{
		std::string labelFolder = encoding;
		...
		for (const auto& dirEntry : fs::directory_iterator(labelFolderPath))
		{
			...
			imagesAndLabels.push_back(std::pair<Mat_<Vec3b>, std::string>(img, encoding));
		}
	}
